...
def run(args: argparse.Namespace, config: Config) -> int:
    # Resolve missing args via stdin so the command works as a friendly REPL.
    utensil_id_arg = args.id
    url = args.url
    if not utensil_id_arg:
        utensil_id_arg = input("Utensil slug (e.g. kadhai-cast-iron): ").strip()
        if not utensil_id_arg:
            log.error("slug is required.")
            return 1
    if not url:
        url = input("Amazon URL or ASIN: ").strip()
        if not url:
