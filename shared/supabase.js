...
// `<meta name="mfc-supabase-publishable-key">` from the host page, creates the
...
(function () {
  ...
  const key = meta('mfc-supabase-publishable-key');

  if (!url || !key) {
    console.error('[mfc] mfc-supabase-url / mfc-supabase-publishable-key meta tags are missing — auth and data calls will fail.');
